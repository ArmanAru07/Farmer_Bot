import React from 'react';

const Home = () => {
    return (
        <div className='relative text-left' style={{ backgroundColor: "#FFFFF2" }}>

            <h1># Working step</h1>

            <h1>1.  Work on therapy page </h1>


            <br /><br />
            <h1># Problems</h1>

            <h1>  In header user name increase, page links positon change. </h1>
            <h1>  The dropdown work in empty page but not in original website. </h1>
            <h1>  when scroll down , header will be small. </h1>
            <h1>  404 page background color. </h1>

            <br /><br />
            <h1># Update</h1>

            <h1> Give a small div to go to "my cart" in shop book option. </h1>
        </div>
    );
};

export default Home;