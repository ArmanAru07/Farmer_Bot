...
const PostCard = ({ index, postInfo }) => {
    ...
    return (
        ...
        <Link key={index} to={`/fullPost/${postInfo._id}`} className={` hover:bg-gray-100 flex flex-col items-${index % 2 === 0 ? 'start' : 'end'} justify-center p-8 text-center bg-white border border-gray-200 rounded-lg shadow-lg dark:border-gray-700 md:mb-2 md:grid-cols-2 text-left dark:bg-gray-800 dark:border-gray-700`}>
            {
                index % 2 === 0 ?
                    <div className="flex items-center justify-center mb-2 lg:mb-4">
                        ...
                        <div className="space-y-0.5 font-medium dark:text-white text-left rtl:text-right ml-3">
                            ...
                            <div className='flex'>
                                ...
                                <div className="flex items-center justify-center ml-3 text-sm text-gray-500 dark:text-gray-400"><FcComments /><span className='ml-1'>{postInfo?.comments?.length ? postInfo?.comments?.length : "0"}</span></div>
                            </div>
                        </div>
                    </div>
                    :
                    <div className="flex items-center justify-center mb-4 lg:mb-4">
                        <div className="space-y-0.5 font-medium dark:text-white text-right  rtl:text-right mr-3">
                            <div>{postInfo?.Username}</div>
                            <div className='flex'>
                                <div className="flex items-center justify-center mr-3 text-sm text-gray-500 dark:text-gray-400"><span className='mr-1'>{postInfo?.comments?.length ? postInfo?.comments?.length : "0"}</span><FcComments /></div>
                                <div className="text-sm text-gray-500 dark:text-gray-400">{formattedDate}</div>
                            </div>
                        </div>
                        <img className="rounded-full w-9 h-9" src="https://cdn1.iconfinder.com/data/icons/avatar-flat-design-outstanding-occupation/512/avatar_fisherman-1024.png" alt="profile picture" />
