        ...
        <div style={{ backgroundColor: '#111827' }}>
            ...
            <div className="min-h-screen p-6  flex items-center justify-center mb-1 pb-20">
                ...
                <div className="container max-w-screen-lg mx-auto">
                    ...
                    <h2 className="font-semibold text-3xl mb-1 text-white">Screening of suicidal behaviour (SBQ-R)</h2>
                    ...
                    <div style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0.5)), url(${image})`, backgroundSize: 'cover' }} className="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6 mt-10">
                        <form onSubmit={handleForm} className="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-8 text-left">
                            ...
                            <div className="mt-10 md:col-span-8">
                                <label htmlFor="address" className='text-base text-white'>Have you ever thought about or attempted to kill yourself? </label>
                                <select name="thought1" className="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                                    <option value="0">Please select one…</option>
                                    <option value="1">Never</option>
                                    <option value="2">It was just a brief passing thought</option>
                                    <option value="3">I have had a plan at least once to kill myself but did not try to do it</option>
                                    <option value="3">I have had a plan at least once to kill myself and really wanted to die</option>
                                    <option value="4">I have attempted to kill myself‚ but did not want to die</option>
                                    <option value="4">I have attempted to kill myself‚ and really hoped to die</option>

                                </select>
                            </div>

                            <div className="mt-10 md:col-span-8">
                                <label htmlFor="address" className='text-base text-white'>How often have you thought about killing yourself in the past year?  </label>
                                <select name="thought2" className="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                                    <option value="0">Please select one…</option>
                                    <option value="1">Never</option>
                            ...
                            <div className="mt-10 md:col-span-8">
                                <label htmlFor="address" className='text-base text-white'>Have you ever told someone that you were going to commit suicide‚ or that you might, do it?   </label>
                                <select name="commit" className="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                                    <option value="0">Please select one…</option>
                                    <option value="1">No</option>
                                    <option value="2">Yes‚ at one time‚ but did not really want to die</option>
                                    <option value="2">RYes‚ at one time‚ and really wanted to die</option>
                                    <option value="3">Yes‚ more than once‚ but did not want to do it</option>
                                    <option value="3">Yes‚ more than once‚ and really wanted to do it</option>
                                </select>
                            </div>

                            <div className="mt-10 md:col-span-8">
                                <label htmlFor="address" className='text-base text-white'>How likely is it that you will attempt suicide someday?  </label>
                                <select name="attempt" className="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                    ...
                    <div style={{ backgroundColor: "#278BC7" }} className='w-3/5 text-white font-semibold  h-40 rounded-md shadow-lg p-3  mb-6'>

                        {level && <h1 className={`mt-2  text-2xl text-${color ? color : ''}-400`}>You have {level}</h1>}
                        {comment && <h1 className='mt-4  text-2xl text-red-400'>{comment}</h1>}
